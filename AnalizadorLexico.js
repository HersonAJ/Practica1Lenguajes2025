...
class AnalizadorLexico {
    ...
    analizar() {
        while (this.posicion < this.texto.length) {
            ...
            if (esLetra(char)) {
                ...
                this.agregarToken("Operador", char);
                this.posicion++;
            } else if (esPuntuacion(char)) {
                this.agregarToken("Puntuacion", char);
                this.posicion++;
            } else if (esAgrupacion(char)) {
                this.agregarToken("Agrupacion", char);
                this.posicion++;
            } else if (char === ' ' || char === '\n') {
                this.posicion++;
            } else {
                this.errores.push(new ErrorLexico(char, this.posicion));
                this.posicion++;
            }
        }
    }
    ...
    analizarIdentificador() {
        ...
        this.agregarToken("Identificador", valor);
    }
    ...
    analizarNumero() {
        ...
        const valor = this.texto.substring(inicio, this.posicion);
        this.agregarToken("Numero", valor);
    }

    /*
    //esto debe seguir comentado
    agregarToken(tipo, valor) {
        this.tokens.push(new Token(tipo, valor, this.posicion));
    }

    agregarToken(tipo, valor, columna, fila) {
        this.token.push(new Token(tipo, valor, columna, fila));
    }
